...
{\
  ...
  if(TENSOR##Contg){                                                         \
    TYPE *TENSOR##_data = NULL;         \
    PRAGMA( omp parallel for if (TENSOR##Size > OMP_THRESHOLD) private(TENSOR##_data,  iter) reduction(OPERATION) ) \
...
{                                                                                              \
  ...
  if( CONTIG1 && CONTIG2 ){                                                                    \
    ...
    if(tp != rp) {                                                                             \
      PRAGMA(ivdep) \
      PRAGMA( omp parallel for if (SIZE > OMP_THRESHOLD) firstprivate(rp, tp)) \
...
{                                                                             \
  ...
  if(CONTIG1 && CONTIG2 && CONTIG3){                                                                    \
    ...
    if (rp != tp) { \
      PRAGMA(ivdep) \
      PRAGMA( omp parallel for if (SIZE > OMP_THRESHOLD) )  \
      for (iter = 0; iter < SIZE; iter++) {\
        TYPE1 *TENSOR1##_data = rp+iter;\
        TYPE2 *TENSOR2##_data = tp+iter; \
