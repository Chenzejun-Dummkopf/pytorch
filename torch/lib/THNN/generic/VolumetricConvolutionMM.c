          ...
          long outputHeight)
{
  ...
  int in_omp = omp_in_parallel();

  #pragma omp parallel if (!in_omp)
  {
    ...
    int w = line_index_offset % inputWidth + pW;
    int h_index = line_index_offset / inputWidth;
    int h = h_index % inputHeight + pH;
    int d_index = h_index / inputHeight;
    int d = d_index % inputDepth + pT;
    int c = d_index / inputDepth;

    int outputHW = outputHeight * outputWidth;
    int outputDHW = outputDepth * outputHW;
    int kHkW = kH*kW;
    int kTkHkW = kT*kHkW;

    register int coeff_d_col = outputHW - dT * kHkW * outputDHW;
    register int coeff_h_col = outputWidth - dH * kW * outputDHW;
    register int coeff_w_col = (1 - dW * outputDHW);
          ...
          long outputHeight)
{
  ...
  #pragma omp parallel if (!in_omp)
  {
    size_t num_threads = omp_get_num_threads();
    size_t tid = omp_get_thread_num();
    long n = nInputPlane*kT*kH*kW;
    int seg_len_tmp = n / num_threads;
    ptrdiff_t line_index_offset = tid * seg_len_tmp;
    ptrdiff_t line_seg_len = (tid == num_threads - 1)? (n-line_index_offset) : seg_len_tmp;
